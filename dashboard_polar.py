...
def compute_metrics(df_polar, df_glucose, window_minutes):
    ...
    if not df_glucose.empty and "sgv" in df_glucose.columns:
        ...
        # compute delta from last two points (mg/dL per min)
        if len(df_glucose) >= 2:
...
# === Combined Plot (no green zone) ===
def create_combined_plot(df_polar, df_glucose):
    fig = go.Figure()

    # Y-range for glucose axis (rightmost)
    y_min, y_max = (40, 180)
    if not df_glucose.empty and "sgv" in df_glucose.columns:
...
def render_live_cards(hr, delta_hr, hrv, delta_hrv, gl, gl_delta, gl_dir, window_minutes):
    ...
    <div class="metric-container">
        ...
        <div class="metric-card accent accent-red">
            <div class="metric-live"><div class="dot dot-red"></div>Live</div>
            <div class="metric-header"><span>❤️ HEART RATE</span></div>
            <div class="metric-value">{safe_format(hr, 0)} <span class="metric-unit">bpm</span></div>
            <div class="metric-delta">{'↑' if (delta_hr or 0) > 0 else '↓' if (delta_hr or 0) < 0 else '→'} {safe_format(delta_hr, 1)} vs mean({window_minutes}m)</div>
        </div>

        <!-- HRV -->
        <div class="metric-card accent accent-blue">
            <div class="metric-live"><div class="dot dot-blue"></div>Live</div>
            <div class="metric-header"><span>💗 HRV (RMSSD)</span></div>
            <div class="metric-value">{safe_format((hrv*1000) if hrv is not None else None, 0)} <span class="metric-unit">ms</span></div>
            <div class="metric-delta">{'↑' if (delta_hrv or 0) > 0 else '↓' if (delta_hrv or 0) < 0 else '→'} {safe_format(delta_hrv, 1)} vs mean({window_minutes}m)</div>
        </div>

        <!-- Glucose -->
        <div class="metric-card accent accent-green">
            <div class="metric-live"><div class="dot dot-green"></div>Live</div>
            <div class="metric-header"><span>🩸 GLUCOSE</span></div>
...
def main():
    ...
    # 1) Live Cards
    render_live_cards(hr, delta_hr, hrv, delta_hrv, gl, gl_delta, gl_dir, window_minutes)

    # 2) Combined Chart (unchanged aesthetic, no green zone)
    ...
    # 5) Glucose Chart (with green target band and expanded Y)
    if not df_glucose.empty and "sgv" in df_glucose.columns:
        ...
        fig_gl.add_trace(go.Scatter(x=df_glucose.index, y=df_glucose["sgv"],
                                    ...
                                    name="Glucose (mg/dL)"))
        g_min, g_max = df_glucose["sgv"].min(), df_glucose["sgv"].max()
        # widen so the green band is clearly visible
